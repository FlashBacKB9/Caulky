...
const SUBS_KEY = 'suscripciones-ids'
export function loadSubIds(): number[] {
  try { return JSON.parse(localStorage.getItem(SUBS_KEY) ?? '[]') } catch { return [] }
}
function saveSubIds(ids: number[]) { localStorage.setItem(SUBS_KEY, JSON.stringify(ids)) }
...
function detectSuggestions(movements: { date: string; dinero: number; movement_type_id?: number | null }[], excluded: Set<number>): number[] {
  ...
  return Object.entries(byType).flatMap(([tid, dates]) => {
    ...
    const ok = (avg >= 6 && avg <= 9) || (avg >= 25 && avg <= 40) || (avg >= 80 && avg <= 105) || (avg >= 340 && avg <= 390)
...
function SubscripcionesSection() {
  ...
  const [subIds, setSubIds]      = useState<Set<number>>(() => new Set(loadSubIds()))
  ...
  const suggestions = useMemo(
    () => detectSuggestions(movements, subIds),
    [movements, subIds]
  )
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-gray-400 dark:text-gray-500">
        Elige qué subtipos de movimiento son suscripciones. Aparecerán en la pestaña Suscripciones del Control de Gastos.
      </p>

      {/* All expense types with toggle */}
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 divide-y divide-gray-50 dark:divide-gray-800">
        ...
        {expenseTypes.map(tp => (
          <div key={tp.id} className="flex items-center gap-3 px-4 py-2.5">
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0">{tp.category}</span>
            <button
              onClick={() => toggle(tp.id)}
              className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none shrink-0 ${subIds.has(tp.id) ? 'bg-blue-500' : 'bg-gray-200 dark:bg-gray-700'}`}
            >
              <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${subIds.has(tp.id) ? 'translate-x-4' : 'translate-x-0.5'}`} />
            </button>
          </div>
        ))}
      </div>
      ...
      {suggestions.length > 0 && (
        <div className="space-y-2">
          ...
          >
            ...
            {suggestions.length} sugerencia{suggestions.length !== 1 ? 's' : ''} detectada{suggestions.length !== 1 ? 's' : ''} automáticamente
