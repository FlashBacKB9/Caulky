...
function GainBadge({ eur, pct }: { eur: number; pct: number }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
      ...
      {pos ? '+' : ''}{eur.toFixed(2)}â‚¬ ({pos ? '+' : ''}{pct.toFixed(2)}%)
    </span>
  )
}

// â”€â”€ Add/Edit Fund form â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <input value={v.ticker} onChange={e => setV(p => ({ ...p, ticker: e.target.value.toUpperCase() }))}
            placeholder="ej. IWDA.AS â€” opcional"
...
// â”€â”€ Purchase row (with inline price/units editing) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (editing) {
    return (
      <tr className="bg-blue-50/30 dark:bg-blue-900/10">
        ...
        <td className="px-3 py-2 text-right text-xs text-gray-300">â€”</td>
        <td className="px-3 py-2 text-right text-xs text-gray-300">â€”</td>
    ...
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800/40 text-gray-600 dark:text-gray-400 cursor-pointer" onClick={onOpen}>
      ...
      <td className="px-3 py-2 text-right text-xs font-mono">
        {p.price_at_purchase != null
          ? <span>{p.price_at_purchase.toFixed(4)} â‚¬</span>
          : fund.ticker
      ...
      <td className="px-3 py-2 text-right text-xs font-mono">
        {p.price_at_purchase != null
          ...
          : p.units != null ? p.units.toFixed(4) : 'â€”'}
      </td>
      <td className="px-3 py-2 text-right text-xs font-mono">
        {currentVal != null
...
// â”€â”€ Fund card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        <>
          ...
          <div className="flex items-center gap-3 px-4 py-3">
            <button onClick={() => setExpanded(e => !e)} className="flex items-center gap-2 flex-1 min-w-0 text-left">
              ...
              {linkedType && <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0">Â· {linkedType.name}</span>}
              ...
              <div className="text-right hidden sm:block">
                ...
                {editingValue ? (
                  ...
                  <button onClick={() => { setValueInput(String(fund.current_value_override ?? fund.current_value ?? '')); setEditingValue(true) }}
                    ...
                    {fund.current_value != null ? fmt(fund.current_value) : <span className="text-gray-300 dark:text-gray-600">â€”</span>}
                    {fund.current_value_override != null && <span className="text-xs text-blue-400 ml-1">âœŽ</span>}
                  </button>
                )}
              </div>
            ...
            {editingPrice ? (
              ...
              <button onClick={() => { setPriceInput(String(fund.current_price ?? '')); setEditingPrice(true) }}
                ...
                {fund.current_price != null
                  ? `${fund.current_price.toFixed(4)} â‚¬/participaciÃ³n`
                  : <span className="text-gray-300 dark:text-gray-600">sin precio â€” click para aÃ±adir</span>}
              </button>
            )}
              ...
              {fund.purchases.length === 0 ? (
                <p className="px-4 py-4 text-xs text-gray-400 dark:text-gray-500">
                  {fund.movement_type_id
                    ? 'Sin movimientos del subtipo vinculado todavÃ­a.'
                    : 'Vincula un subtipo de movimiento para ver las compras aquÃ­.'}
                </p>
              ) : (
                <table className="w-full text-xs">
                  <thead className="bg-gray-50 dark:bg-gray-800/50 text-gray-400 dark:text-gray-500">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-right">Precio/part.</th>
  ...
  return (
    <div className="p-3 md:p-6 space-y-4 md:space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Inversiones</h1>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Las compras se registran como movimientos del subtipo vinculado</p>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Valor actual',     value: hasValue ? fmt(summary.total_current_value!) : 'â€”',                                                                        color: 'text-gray-700 dark:text-gray-200' },
            { label: 'Ganancia â‚¬',       value: hasValue ? `${summary.gain_eur! >= 0 ? '+' : ''}${fmt(summary.gain_eur!)}` : 'â€”',                                         color: !hasValue ? 'text-gray-400' : summary.gain_eur! >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400' },
            { label: 'Rentabilidad',     value: hasValue && summary.gain_pct != null ? `${summary.gain_pct >= 0 ? '+' : ''}${summary.gain_pct.toFixed(2)}%` : 'â€”',        color: !hasValue ? 'text-gray-400' : (summary.gain_pct ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400' },
          ].map(c => (
