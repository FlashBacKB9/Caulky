...
function computeOneProjection(
  ...
  const effectiveAmortPerYear = amortPerYearOverride ?? m2.amortizePerYear
  ...
  for (let t = 0; t <= totalMonths; t++) {
    ...
    if (inPhase2 && hasMortgage && mortgageRemaining > 0) {
      ...
      // Extra amortization
      if (withAmortization && m2.amortize && effectiveAmortPerYear > 0 && balance > m2.amortizeThreshold && mortgageRemaining > 0) {
        const extra = Math.min(effectiveAmortPerYear / 12, mortgageRemaining)
    ...
    points.push({
      ...
      amortExtraCum: hasMortgage ? Math.round(cumulativeExtra) : undefined,
    })
  }
