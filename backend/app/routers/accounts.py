...
def _acc_snap(a: Account) -> dict:
    return {"name": a.name, "color": a.color, "icon": a.icon, "initial_balance": float(a.initial_balance), "category": a.category, "depreciation_rate": float(a.depreciation_rate) if a.depreciation_rate is not None else None, "value_date": str(a.value_date) if a.value_date else None}
...
async def create_account(body: AccountCreate, db: AsyncSession = Depends(get_db), user: User = Depends(current_active_user)):
    ...
    account = Account(
        ...
        depreciation_rate=body.depreciation_rate, value_date=body.value_date,
    )
    db.add(account)
    await db.flush()
